...
const OptimizedLoginForm = ({ onToggleMode }) => {
  ...
  const getErrorMessage = useCallback((result, fallbackMessage) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(getErrorMessage(result, 'Login failed'));
  ...
  const handleDemoLogin = async (type) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(getErrorMessage(result, 'Demo login failed'));
        if (result.attemptsRemaining !== undefined) {
          setAttemptsRemaining(result.attemptsRemaining);
        }
      } else {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(getErrorMessage(result, 'Failed to send reset email'));
      }
    } catch {
      setError('Network error. Please try again.');
    } finally {
