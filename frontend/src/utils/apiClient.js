...
const isCsrfErrorResponse = (response, data) =>
  response.status === 403
  && (
 ...
 */
const request = async (endpoint, { method = 'GET', body, headers = {}, raw = false, _retried = false } = {}) => {
  ...
  if (isMutation && !_retried && isCsrfErrorResponse(response, data)) {
...
const upload = async (endpoint, formData) => {
  ...
  if (isCsrfErrorResponse(response, data)) {
    const freshToken = await fetchCsrfToken();
    if (freshToken) {
